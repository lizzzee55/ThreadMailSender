...
function getEmails()
{
	global $db, $emailCollection, $offset, $maxThread, $end;
	...
	$sql = "SELECT * FROM `emails` RIGHT JOIN `users` USING(`email`)
		WHERE `users`.`email_confirmed` = 1 and `emails`.`check` = 0 and `emails`.`valid` = 0 LIMIT " . $offset . "," . $limit . ";";
	...
	printf("Selected %d emails append: %d email\r\n", count($emailCollection), $n);
...
function createTask()
{
	...
	foreach($emailCollection as $id => $item)
	{
		...
		$process = run_background("checker_instance.php", $base64string);
		if(!$process)
			return 0;
